...
viewof show = Inputs.button("OnDemand", {label: "Show"});

value = valid_book;  // Replace with any reactive logic you'd like

mutable displayedValue = null

update = {
  if (show) {
    mutable displayedValue = value
  }
  null  // avoid displaying anything from this cell
}
...
/*
viewof styledButton = {
  const button = Inputs.button("Click me");
  button.className = "btn btn-primary";

  (async () => {
    for await (const _ of Generators.input(button)) {
      // Toggle class on each click
      button.classList.toggle("btn btn-warning");
    }
  })();
...
/*
viewof time = { 
  const buttn = Inputs.button("Update", {  
    value: null, 
    reduce: () => new Date
    });
  buttn.className = "btn btn-primary";
  return buttn;
};  
*/
...
//  display.insertAdjacentHTML("beforeend", fetchPassage.htmlraw);
