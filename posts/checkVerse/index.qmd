...
async function fetchPassage(param) {
  const url = `https://hall.pjafischer.workers.dev/passage/?param=${param}`;
...
data = await fetchPassage(userRef);

// Display the fetched content
html`<div>${data.htmlraw}</div>`;
```
...
viewof addButton = Inputs.button(html`<button class="btn btn-primary">Add</button>`)
...
//======================================================
//===== CHECK 
function bcve_(input) {
  const editions = new Set(['KJV','SG21',"NGU"]);
  
  // Split by multiple separators: comma, semicolon, and pipe
  const [bc,verse,edition="SG21"] = input.split(/[:!]/);
